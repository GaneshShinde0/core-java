    ...
    public static int removeDuplicates(int[] nums) {
        ...
        for (int i = 1; i < nums.length; i++) {
            if (nums[i] != nums[uniqueIndex]) {
                ...
                // Place it at the next position in the array
                uniqueIndex++;
                nums[uniqueIndex] = nums[i];
            }
        }

        // Length of the unique portion of the array will be (uniqueIndex + 1)
        return uniqueIndex + 1;
    }


    public static void main(String[] args) {
        int[] nums0 = {0, 0, 1, 0, 1, 2, 2, 3, 3, 4};
        int newLength0 = removeDuplicates(nums0);
        System.out.println("Test case 1: New length: " + newLength0);
        System.out.println("Test case 1: Unique values in list: " + Arrays.toString(Arrays.copyOfRange(nums0, 0, newLength0)));
